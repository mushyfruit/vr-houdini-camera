...
class QHandMonitor(QThread):
    ...
    def create_context(self):
        app_info = xr.ApplicationInfo("hand_trac2k", 0, "pyopenxr", 0, xr.XR_CURRENT_API_VERSION)
        self.context = xr.ContextObject(instance_create_info=xr.InstanceCreateInfo(enabled_extension_names=[xr.KHR_OPENGL_ENABLE_EXTENSION_NAME,], application_info = app_info),)

    def run(self):
        app_info = xr.ApplicationInfo("hand_track", 0, "pyopenxr", 0, xr.XR_CURRENT_API_VERSION)
        #self.create_context()
        ...
        with xr.ContextObject(
            ...
            for frame_index, frame_state in enumerate(context.frame_loop()):
                #context.session_state = xr.SessionState.FOCUSED
                avail = xr.enumerate_instance_extension_properties
